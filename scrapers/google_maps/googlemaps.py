...
class GoogleMaps:
    ...
    def __init__(self, n_max_reviews):
        ...
        self.targetfile = open(folder + config['review-file'], mode='a', encoding='utf-8', newline='\n')
        self.writer = self.__get_writer(HEADER)

        self.N = n_max_reviews

        self.driver = self.__get_driver(debug=False)
    ...
    def get_reviews(self, url, type):

        print(f"Saving to {self.targetfile}")
        self.driver.get(url)
        ...
        while not clicked and tries < MAX_RETRY:
            try:
                menu_bt.click()
                # second element of the list: most recent, new, pos, neg
                recent_rating_bt = self.driver.find_elements_by_xpath('//div[@role=\'option\']')[type]
        ...
        # self.driver.find_element_by_xpath("//button[@class='section-expand-review blue-link']").click()  
        ...
        while n_reviews_loaded < self.N and n_scrolls < MAX_SCROLLS:
            ...
            sys.stdout.write(f"Scraped: {n_reviews_loaded}")
            sys.stdout.flush()
    ...
    def __get_driver(self, debug=False):
        ...
        input_driver = webdriver.Chrome("../Scraping-TripAdvisor-with-Selenium-2019/chromedriver", options=options)
